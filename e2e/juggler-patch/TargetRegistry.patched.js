...
export class TargetRegistry {
  ...
  async _newPageInternal({browserContextId}) {
    ...
    // E2E PATCH: a temp-installed WebExtension can spawn an extra tab
    // (addon UI / onInstalled handler) alongside the initial blank, which
    // headed Juggler tolerates but headless Juggler asserted on. Close
    // extras so the remaining invariant (exactly one tab) holds.
    while (window.gBrowser.browsers.length > 1) {
      try {
        window.gBrowser.removeTab(window.gBrowser.tabs[window.gBrowser.tabs.length - 1]);
      } catch (e) { break; }
    }
